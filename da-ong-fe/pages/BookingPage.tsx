...
const BookingPage: React.FC = () => {
  ...
  // Initial State
    const [booking, setBooking] = useState<BookingState>(() => {
        // Nếu có ?step=4 và pendingBooking thì khôi phục
        ...
        if (stepParam === '4') {
            ...
            // Nếu không có booking tạm, chuyển về bước 1
  ...
  // Handlers
    const handleNext = () => {
        // Luôn lưu trạng thái booking mới nhất vào localStorage trước khi chuyển bước
        setBooking(prev => {
            const next = { ...prev, step: prev.step + 1 };
            localStorage.setItem('pendingBooking', JSON.stringify(next));
            if (prev.step === 2) {
                window.location.href = '/menu?fromBooking=1';
            }
            return next;
        });
    };
  ...
  const updateDishQty = (id: string, delta: number) => {
      setBooking(prev => {
          ...
          const newDishes = { ...prev.selectedDishes };
          if (newQty <= 0) delete newDishes[id];
          else newDishes[id] = newQty;
          return { ...prev, selectedDishes: newDishes };
      });
  };
      ...
      <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
                  ...
                  {Object.keys(booking.selectedDishes).length > 0 && (
                      <div className="space-y-2 mb-4">
                          <h4 className="font-bold text-gray-700">Món đã chọn:</h4>
                          {Object.entries(booking.selectedDishes).map(([id, qty]) => {
                               const d = menuItemsToUse.find(i => String(i.id) === String(id));
                               return (
                                   <div key={id} className="flex justify-between text-sm">
                                       <span>{d ? d.name : `Món #${id}`} <span className="text-gray-500">x{qty}</span></span>
                                       <span>{d ? (d.price * (qty as number)).toLocaleString() + 'đ' : ''}</span>
                                   </div>
                               )
                          })}
                      </div>
