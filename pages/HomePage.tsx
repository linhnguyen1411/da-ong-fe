...
import { ArrowRight, Loader2, ZoomIn, ZoomOut, RotateCcw, X, ChevronLeft, ChevronRight, Tag } from 'lucide-react';
import DishCard from '../components/DishCard';
import { getBestSellers, getRooms, getPromotions, ApiRoom, ApiMenuItem, ApiPromotion, API_BASE_ORIGIN } from '../services/api';
...
const HomePage: React.FC = () => {
  ...
  const [promoHighlight, setPromoHighlight] = useState<ApiPromotion | null>(null);
  ...
  useEffect(() => {
    const fetchPromo = async () => {
      try {
        ...
        const highlighted = (data || []).find((p) => p.highlighted);
        setPromoHighlight(highlighted || null);
      } catch {
        setPromoHighlight(null);
      }
    };
    fetchPromo();
  }, []);
    ...
    <div className="min-h-screen">
      ...
      <section className="py-20 bg-light">
          <div className="container mx-auto px-4 grid md:grid-cols-3 gap-8 text-center">
              <div className="p-6">
                  <h3 className="text-2xl font-sans font-bold mb-3 text-dark">Nguyên Liệu Tươi Ngon</h3>
               ...
               <div className="p-6 border-l border-r border-gray-200">
                  <h3 className="text-2xl font-sans font-bold mb-3 text-dark">Đầu Bếp 5 Sao</h3>
                  <p className="text-gray-600">Đội ngũ đầu bếp hàng đầu với 20 năm kinh nghiệm.</p>
              </div>
               <div className="p-6">
                  <h3 className="text-2xl font-sans font-bold mb-3 text-dark">Không Gian Riêng Tư</h3>
                  <p className="text-gray-600">Hệ thống phòng VIP sang trọng và sân vườn thoáng đãng.</p>
              </div>
          </div>
      </section>

      {/* Ưu đãi / Khuyến mãi - menu chương trình */}
      <section className="py-14 bg-white border-y border-gray-100">
        <div className="container mx-auto px-4">
          <Link
            to="/uu-dai"
            className="group flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6 md:p-8 rounded-2xl bg-gradient-to-r from-primary/10 to-amber-50 border border-primary/20 hover:border-primary/40 hover:shadow-lg transition-all"
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <span className="text-primary font-bold tracking-wider text-sm uppercase">Deal hot</span>
                <h3 className="text-xl md:text-2xl font-sans font-bold text-dark mt-0.5">
                  {promoHighlight ? promoHighlight.title : 'Chương trình ưu đãi'}
                </h3>
                <p className="text-gray-600 text-sm mt-1">
                  {promoHighlight
                    ? 'Đang diễn ra · Xem chi tiết và điều kiện áp dụng'
                    : 'Xem các ưu đãi đang có và sắp diễn ra'}
                </p>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <div>
              ...
              <h2 className="text-4xl font-sans font-bold text-dark mt-2">Sơ Đồ Quán & Phòng</h2>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4">
              <div className="flex justify-between items-end mb-12">
                  <div>
                      ...
                      <h2 className="text-4xl font-sans font-bold text-dark mt-2">Thực Đơn Nổi Bật</h2>
                  </div>
