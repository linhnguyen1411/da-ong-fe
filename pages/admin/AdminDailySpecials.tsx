...
import {
  adminGetDailySpecials, adminCreateDailySpecial, adminUpdateDailySpecial, adminDeleteDailySpecial,
  adminToggleDailySpecialPin, adminToggleDailySpecialHighlight, adminUploadDailySpecialImages,
  adminDeleteDailySpecialImage,
  adminGetMenuItems,
  ApiDailySpecial, ApiMenuItem
} from '../../services/api';
import {
  Plus, Edit2, Trash2, Loader2, X, Save, Pin, Sparkles, Calendar, Upload, Image, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const AdminDailySpecials: React.FC = () => {
  const [items, setItems] = useState<ApiDailySpecial[]>([]);
  const [menuItems, setMenuItems] = useState<ApiMenuItem[]>([]);
  const [loading, setLoading] = useState(true);
  
  // Modal state
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<ApiDailySpecial | null>(null);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [specialsData, menuItemsData] = await Promise.all([
        adminGetDailySpecials(),
        adminGetMenuItems()
      ]);
      setItems(specialsData);
  ...
  const openEditModal = async (item: ApiDailySpecial) => {
    try {
      const detail = await adminGetDailySpecials().then(list => list.find(i => i.id === item.id));
      if (detail) {
        ...
        setFormData({
          ...
          special_date: detail.special_date || new Date().toISOString().split('T')[0],
        ...
        setFormData({
          ...
          special_date: item.special_date || new Date().toISOString().split('T')[0],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingItem) {
        await adminUpdateDailySpecial(editingItem.id, dataToSend);
      } else {
        await adminCreateDailySpecial(dataToSend as any);
      }
      setShowModal(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleDelete = async (item: ApiDailySpecial) => {
    const menuName = item.title || item.menu_item?.name || 'món này';
    if (!confirm(`Bạn có chắc muốn xóa "${menuName}"?`)) return;

    try {
      await adminDeleteDailySpecial(item.id);
      fetchData();
    } catch (err: any) {
      alert('Lỗi: ' + err.message);
    }
  ...
  const handleTogglePin = async (item: ApiDailySpecial) => {
    try {
      await adminToggleDailySpecialPin(item.id);
      fetchData();
    } catch (err: any) {
      alert('Lỗi: ' + err.message);
    }
  ...
  const handleToggleHighlight = async (item: ApiDailySpecial) => {
    try {
      await adminToggleDailySpecialHighlight(item.id);
      fetchData();
    } catch (err: any) {
      alert('Lỗi: ' + err.message);
    }
  ...
  const handleUploadImages = async (item: ApiDailySpecial, files: FileList) => {
    try {
      setUploadingImages(true);
      await adminUploadDailySpecialImages(item.id, Array.from(files));
      fetchData();
    } catch (err: any) {
  ...
  const getItemImages = (item: ApiDailySpecial): string[] => {
    const images: string[] = [];
  ...
  const getDisplayImage = (item: ApiDailySpecial): string => {
  ...
  if (loading) {
    return (
      <AdminLayout title="Quản lý Món ngon mỗi ngày">
    ...
    <AdminLayout title="Quản lý Món ngon mỗi ngày">
      ...
      {items.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-gray-500">Chưa có món ngon nào.</p>
          <button
            onClick={openCreateModal}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-xl font-bold text-dark">
                {editingItem ? 'Sửa món ngon' : 'Thêm món ngon mỗi ngày'}
              </h2>
            ...
            <div className="p-6 space-y-4">
              ...
              {editingItem && (
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Ảnh món ngon mỗi ngày</label>
                      ...
                      <div key={img.id} className="relative group">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            // Gọi API xoá ảnh
                            await adminDeleteDailySpecialImage(editingItem.id, img.id);
                            // Cập nhật lại editingItem sau khi xoá
                            const updated = await adminGetDailySpecials();
                    ...
                    <label className="w-24 h-24 flex items-center justify-center border-2 border-dashed rounded cursor-pointer hover:border-primary text-gray-400 hover:text-primary">
                      ...
                      <input
                        ...
                        onChange={async (e) => {
                          if (e.target.files) {
                            setUploadingImages(true);
                            await adminUploadDailySpecialImages(editingItem.id, Array.from(e.target.files));
                            const updated = await adminGetDailySpecials();
                            const found = updated.find(i => i.id === editingItem.id);
                            if (found) setEditingItem(found);
                            setUploadingImages(false);
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Ngày đặc biệt</label>
                <input
                  type="date"
...
export default AdminDailySpecials;
