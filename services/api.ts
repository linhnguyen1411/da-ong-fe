...
// Recruitments
export const getRecruitments = () => apiCall<ApiRecruitment[]>('/recruitments');
export const getRecruitment = (id: number) => apiCall<ApiRecruitment>(`/recruitments/${id}`);

// Promotions
export const getPromotions = () => apiCall<ApiPromotion[]>('/promotions');
export const getPromotion = (id: number) => apiCall<ApiPromotion>(`/promotions/${id}`);
...
/** Gửi liên hệ kèm file đính kèm (multipart/form-data) */
export async function submitContactWithAttachments(formData: FormData): Promise<{ message: string; contact: ApiContact }> {
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error || err.errors?.join(', ') || `HTTP ${response.status}`);
  }
...
// Admin Recruitments
export const adminGetRecruitments = () =>
  apiCall<ApiRecruitment[]>('/admin/recruitments', { headers: getAuthHeader() });

export const adminCreateRecruitment = (data: Partial<ApiRecruitment>) =>
  apiCall<ApiRecruitment>('/admin/recruitments', {
    method: 'POST',
    headers: getAuthHeader(),
    body: JSON.stringify(data),
  });

export const adminUpdateRecruitment = (id: number, data: Partial<ApiRecruitment>) =>
  apiCall<ApiRecruitment>(`/admin/recruitments/${id}`, {
    method: 'PATCH',
    headers: getAuthHeader(),
    body: JSON.stringify(data),
  });

export const adminDeleteRecruitment = (id: number) =>
  apiCall<void>(`/admin/recruitments/${id}`, {
    method: 'DELETE',
    headers: getAuthHeader(),
  });

// Admin Promotions
export const adminGetPromotions = () =>
  apiCall<ApiPromotion[]>('/admin/promotions', { headers: getAuthHeader() });

export const adminCreatePromotion = (data: Partial<ApiPromotion>) =>
  apiCall<ApiPromotion>('/admin/promotions', {
    method: 'POST',
    headers: getAuthHeader(),
    body: JSON.stringify(data),
  });

export const adminUpdatePromotion = (id: number, data: Partial<ApiPromotion>) =>
  apiCall<ApiPromotion>(`/admin/promotions/${id}`, {
    method: 'PATCH',
    headers: getAuthHeader(),
    body: JSON.stringify(data),
  });
...
export const adminDeletePromotion = (id: number) =>
  apiCall<void>(`/admin/promotions/${id}`, {
    method: 'DELETE',
    headers: getAuthHeader(),
  });
